...
- [`weeks/09-going-further/`](weeks/09-going-further/README.md) is an optional module, not a required week, for anyone who wants to close one specific gap: classes and type hints are never covered in Weeks 0-8, on purpose, which means real, typed Python (pydantic, most ORMs, the typed objects most SDKs actually return) is a wall rather than a stretch right now. Three short notes and one unguided exercise, built to close exactly that gap and nothing more.

...
<p>Nothing here assumes you know blockchain already. By the end, you'll understand what a hash chain actually protects against, why proof of work and proof of stake exist as competing answers to the same problem, and how systems like Bitcoin and Ethereum work under the hood, because you'll have built simplified versions of each piece yourself. Level 6 then takes this chain and makes it production-real, with proper cryptography, wallets, and networked nodes.</p>`,
    starterCode: ""
  },
  {
    id: 200,
  ...
  <div class="io-row"><span class="io-key">Effect</span><code class="io-val">the 50-coin UTXO is consumed; Bob gets a new 40-coin UTXO; Alice gets a new 10-coin change UTXO (her other 30-coin UTXO is untouched)</code></div>
    ...
    validation: {
      ...
      pyTests: [
        ...
        { code: "result3 = spend_utxos(utxo_set, 'Alice', 'Carol', 50, 'tx3')\nassert result3 == False", message: "Alice's original 50-coin UTXO is already gone; a later spend that would need it again (more than her real remaining 40) should fail, not silently double-spend the consumed coin." }
      ]
    },
...
<p>The order matters: a block can only be mined once you know exactly which transactions belong in it, and you only know that once you've checked which ones are actually valid. First verify and apply transactions (only the valid ones actually happened), then summarize the accepted ones into a Merkle root (so the block has one fingerprint for its whole batch), then mine, searching for a nonce that makes the block's hash meet the difficulty target, just hashing the Merkle root instead of a plain data string this time.</p>
<span class="task-label">Your Task</span>

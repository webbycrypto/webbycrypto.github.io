...
<div class="note-block">
  <span class="note-label">Note</span>
  <span>Checked on the shape of the template you write, the same as the rest of this section's Jinja content: the required tags and structure, not the exact rendered whitespace.</span>
</div>`,
    hints: [
    ...
    explanation: `<p>This is the actual day-to-day shape of writing a Jinja template: mostly literal HTML, with a few well-placed tags marking exactly where it needs to flex. <code>**profile</code> means the <code>.render()</code> call itself never needs editing when a new field gets added to <code>profile</code> elsewhere in the app: every key in the dict becomes an available template variable automatically, even though you'd still edit the template to actually display a new one. Escaping <code>bio</code> here is the manual version of something a real FastAPI project's Jinja setup does for you by default, which Level 6 covers.</p>`
  },
  {
    id: 255,
